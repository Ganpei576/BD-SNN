...
def non_max_suppression(    #yolov3中的nms函数
    prediction,
    conf_thres=0.25,
    iou_thres=0.45,
    classes=None,
...
def non_max_suppression_source(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,    #原本的NMS函数
                        labels=(), max_det=300):
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    ...
    if path.exists() and not exist_ok:
        ...
        dirs = glob.glob(f"{path}{sep}*")  # 使用 glob 模块查找与 path 类似的路径，返回匹配的路径列表
